...
const allListElements = document.querySelectorAll('ul, ol'); 

allListElements.forEach(listElement => {
  // If this list is already within a card, skip it
  if (listElement.closest('.content-card')) return; 

  // Create a new card container
  const cardContainer = document.createElement('div');
  cardContainer.classList.add('content-card');

  // Insert the card container before the first list
  listElement.parentNode.insertBefore(cardContainer, listElement);

  // Group consecutive lists
  let currentList = listElement;
  while (currentList && (currentList.tagName === 'UL' || currentList.tagName === 'OL')) {
    cardContainer.appendChild(currentList);
...
document.addEventListener('click', function(event) {
  // Check if the viewport width is 768px or less (common breakpoint for mobile devices)
